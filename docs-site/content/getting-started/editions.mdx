...
| SSO                                 | No             | Yes (OIDC)                | Yes                           |
| RBAC                                | No             | Yes                       | Yes                           |
| Inline multi-channel fan-out (API)  | Yes            | Yes                       | Yes                           |
| Saved broadcast groups + admin UI   | No             | Yes                       | Yes                           |

Inline fan-out uses `broadcast_group` on send routes (API only — not in the web UI).
Saved groups, group CRUD, and the Broadcast Groups admin UI require Enterprise.
