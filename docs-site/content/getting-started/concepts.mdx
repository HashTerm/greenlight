...
sequenceDiagram
  ...
  participant Human
  Agent->>Greenlight: POST /register-channel (PROMPT)
  Agent->>Greenlight: POST /v1/prompts "Deploy v2.4 to prod?"
...
which can reply with `POST /send`.
...
sequenceDiagram
  ...
  participant Greenlight
  participant Agent
  Agent->>Greenlight: POST /register-channel (MESSAGE)
...
| **MESSAGE**  | Outbound via `POST /send` from any client | **No** — one channel `callback_url` receives all chat |
