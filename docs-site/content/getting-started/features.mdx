...
- **Bidirectional chat on MESSAGE channels** — inbound messages forward to your
  `callback_url`; replies go out with `POST /send`.
- **Admin UI** — manage channels and prompts, view dashboard stats, and send
  test messages on MESSAGE channels.
...
Register a channel once (`POST /register-channel`), then pass that `channel_id`
...
flowchart LR
  ...
  w1 -->|"POST /v1/prompts (callback A)"| gl
  w2 -->|"POST /v1/prompts (callback B)"| gl
  gl --> chat
  chat -->|answer| gl
...
| Type        | Agent → human              | Human → agent                                      | Callback                                  |
| ----------- | -------------------------- | -------------------------------------------------- | ----------------------------------------- |
| **PROMPT**  | `POST /v1/prompts`         | Prompt answers only (buttons or `ID:#123` text)    | Per-prompt `callback_url` (signed)        |
| **MESSAGE** | `POST /send`               | Any chat text                                      | Single channel `callback_url` (unsigned)  |
