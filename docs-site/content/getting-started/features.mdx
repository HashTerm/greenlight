...
- **Multi-channel fan-out** — send one prompt or message to many channels via API
  (`broadcast_group` inline object); Enterprise adds saved broadcast groups and
  admin UI. See [Broadcast Groups](/guides/broadcast-groups).
...
- **Admin UI** — manage prompts, messages, and channels; view dashboard stats;
...
| ----------- | ------ | ------------------------ | --------------------------------------- | ---------------------------------------- |
| **PROMPT**  | `new`  | `POST /v1/prompts/new`   | Buttons, Type answer, or `ID:#123` text | Per-prompt `callback_url` (signed)       |
...
- **PROMPT is not a general chat router** — arbitrary text on a PROMPT channel is
  ignored unless the user taps **Type answer** or uses `ID:#n` text syntax.
...
- **Shared Agent API key** — one `GREENLIGHT_API_KEY` for the Agent API today (no per-agent
  keys).
...
| Fan out prompts or messages        | [Broadcast Groups](/guides/broadcast-groups)         |
