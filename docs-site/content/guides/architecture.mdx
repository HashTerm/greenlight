...
flowchart LR
  agent[AI_Agent]
  ui[Admin_UI]
  core[Greenlight_Core]
  db[(Postgres)]
  platforms[Chat_Platforms]

  agent -->|X-API-Key| core
  ui -->|X-API-Key_and_X-Admin-Token| core
  core --> db
  core <-->|Chat_SDK_adapters| platforms
  core -->|signed_prompt_callbacks| agent
  core -->|message.created| agent
  platforms -->|webhooks_or_polling| core
```
