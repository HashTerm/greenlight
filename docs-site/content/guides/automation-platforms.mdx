...
**Outbound (automation → Greenlight)** — register channels, create prompts, send
messages. Authenticate with `X-API-Key` when `USE_AUTH=true`.
...
1. **Trigger** — GitHub Actions, GitLab CI, Jenkins, or a schedule fires your workflow.
2. **Create prompt** — `POST /v1/prompts` with `options`, `correlation_id` (build ID), and a webhook URL from your automation tool.
3. **Wait for human** — Greenlight delivers the card to Slack/Telegram/etc.
4. **Resume workflow** — On signed callback, branch on `answer.value` (`Approve` / `Reject`) and continue the pipeline or open a ticket.
...
sequenceDiagram
  ...
  Auto->>Greenlight: POST /v1/prompts
...
1. **Register once** — `POST /register-channel` with `channel_type: "MESSAGE"` and the automation tool's webhook URL as `callback_url`.
2. **On message** — Handle unsigned `message.created` events; call an LLM API, lookup a runbook, or update a ticket.
3. **Reply** — `POST /send` back through Greenlight to the same chat.
...
| ----------------- | --------------------------------------------------------- | ---------------------------------------------------- |
| Register channel  | `channel_type: "PROMPT"`                                  | `channel_type: "MESSAGE"` + `callback_url`           |
| Start interaction | `POST /v1/prompts` (+ optional per-prompt `callback_url`) | Human sends chat message                             |
| Receive event     | Signed `POST` to `callback_url`                           | Unsigned `message.created` to channel `callback_url` |
| Verify            | HMAC-SHA256 `X-Signature` header                          | Optional; not signed by Greenlight                   |
| Respond in chat   | Usually not needed (human already answered)               | `POST /send`                                         |
...
**Create a prompt (any platform's HTTP action):**
...
- [Channels](/guides/channels) — register-channel schema
