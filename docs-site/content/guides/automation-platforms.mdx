...
2. **Configure the platform** — point the chat platform at Greenlight's inbound
   webhook: `{PUBLIC_WEBHOOK_URL}/webhooks/{platform}/{channel_id}` (see
...
| Platform setup    | Usually none (Telegram polls by default)                                         | Set `{PUBLIC_WEBHOOK_URL}/webhooks/{platform}/{channel_id}` in the platform app |
...
`{PUBLIC_WEBHOOK_URL}/webhooks/{platform}/{channel_id}` — see
[Platforms](/platforms).
