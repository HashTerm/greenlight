...
3. **Create prompt** — `POST /v1/prompts/new` with `options`, `correlation_id` (short label), optional `callback_data` (JSON workflow state), and a webhook URL from your automation tool.
4. **Wait for human** — Greenlight delivers the card to Slack/Telegram/etc.
5. **Resume workflow** — On signed callback, read `channel_id`, `callback_data`, and `answer.value` to branch the pipeline or open a ticket.

For approvals that must reach **multiple** PROMPT channels in one step, use inline
`broadcast_group` on `POST /v1/prompts/new` (API only — community and enterprise):
...
```json
{
  ...
  "callback_url": "https://your-n8n.example/webhook/approve"
}
```

Enterprise operators can also create saved broadcast groups and send with
...
### Workflow A → human → workflow B

Split automation across two workflows when the first only creates the prompt and
the second handles the answer:

1. **Workflow A** — `POST /v1/prompts/new` with `channel_id`, `correlation_id`,
   `callback_data` (JSON state your second workflow needs), and `callback_url`
   pointing at Workflow B's webhook.
2. **Human** — answers in chat (button or Type answer).
3. **Workflow B** — receives the signed callback with `channel_id`,
   `callback_data`, and `answer`; resumes deploy, ticketing, or LLM steps.

Each PROMPT channel has its own `#n` ids — always pass `channel_id` when fetching
prompts via `GET /v1/prompts/{id}`.
